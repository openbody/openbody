...
- **Specification docs** (this repo, `LICENSE`) — OWFa 1.0 (Granted Claims) — i.e. the
  ...
  Specification identified as `OpenBody 0.9.0` and an individual grant executed. ✅
...
for `OpenBody 0.9.0` on 2026-07-05, superseding the executions for `OpenBody 0.8.0`
(2026-07-03), `OpenBody 0.6.0` (2026-07-01) and, before that, the initial `OpenBody 0.3.1`.)

> **Release-process note.** The OWFa grant identifies a specific spec version. Re-execute
> it — the version line and date in `LICENSE`, plus the references in this file — as part
> of every release that bumps the spec version, so the grant never lags the published spec.
