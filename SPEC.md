...
| `phases` | optional | array&lt;`Phase`&gt; | Named macro/meso/microcycle grouping over the program's sessions (e.g. "Hypertrophy," "Strength," "Peaking," "Deload"). Each `Phase` is a sub-object (not a separate record): `name` (required string), `weekStart`/`weekEnd` (optional integers, 1-based, inclusive — see anchor definition below), `goal` (optional string), `sessions` (optional array&lt;ref&gt; — ordered `Session` references belonging to that phase, same reference semantics as the top-level `sessions` field). |

A Program is typically a *planned* artifact that performed Sessions link back to
(§5.6, §7).

**Week-numbering anchor:** `weekStart`/`weekEnd` are 1-based and relative to the
Program's earliest referenced `Session.startTime` — week 1 is the week containing
whichever `Session` referenced by this `Program` (via `sessions` and/or
`phases[].sessions`) has the earliest `startTime`; week 2 is the following
calendar week from that anchor, and so on. This is the only week-numbering
convention defined by this spec; no other field shares or presumes it.
...
**Disjointness and ordering:** a session id **MUST NOT** appear in more than one
phase's `sessions` array — phases are disjoint. This matters most in
phases-only mode (no top-level `sessions`, below), where the program's effective
session list is the concatenation of `phases[].sessions` in phase order; an
overlapping id would otherwise be silently duplicated in that concatenation with
no defined tie-break. When both `Program.sessions` and `phases` are present, a
phase's `sessions` order **MUST** be consistent with the top-level
`Program.sessions` order — that is, a phase's own `sessions` array **MUST**
appear as a contiguous, order-preserving subsequence of `Program.sessions`, not
an arbitrarily-reordered subset.
...
organized under `phases`) rather than relying on `recurrence`; a `Program`
**MUST NOT** carry both `recurrence` and `phases` — the two are mutually
exclusive.
...
| `setRole` | optional | token | Open, purely **structural**: `warmup｜working｜drop｜backoff｜…` (R1/R6); has a tail (cluster/myo/primer). Does **not** encode terminal state (e.g. "taken to failure") — see `terminatedBy` below. |
...
| `terminatedBy` | token | *`performance` only* — how the atom actually **ended** (a performed-side fact). Open token mirroring `Target.stopCondition.kind` (§5.10) where the same concept applies on the performed side (e.g. `to_failure｜to_rpe｜…`), plus performed-only tokens such as `voluntary｜time_cap｜…`. Orthogonal to `setRole`: `setRole` is the set's *structural* position, `terminatedBy` is *how it ended*; both **MAY** be present on one `WorkUnit` (see below). |
...
relative to other sets (`warmup｜working｜drop｜backoff｜…`, or a tail role like
cluster/myo/primer). `terminatedBy` is a `performance`-only field recording **how the
set actually ended** — a `to_failure` stop, a stop at a given RPE/RIR, a voluntary
stop, a time cap, etc. — mirroring the planned-side `Target.stopCondition.kind`
(§5.10) but as a **performed fact** rather than a target. A set's structural role
never determines how it ended: a drop-set component, a rest-pause mini-set, or a
myo-rep activation set **MAY** independently be taken to failure or not. This is
expressed as two fields on one `WorkUnit`, e.g. `setRole: "drop"` +
`performance.terminatedBy: "to_failure"` — never by overloading `setRole` with a
...
- **Open, registry-backed token** — has a demonstrated long tail; uses the §4.5/§6
  ...
  (canon `warmup｜working｜drop｜backoff`, with a tail — cluster/myo/primer roles;
  purely structural, R1/R6), `terminatedBy` (§5.5; performed-side terminal state,
  mirrors `Target.stopCondition.kind` where applicable, e.g.
  `to_failure｜to_rpe｜voluntary｜time_cap｜…`).
